...
function StatCard({ stat, index }: { stat: ListingStat; index: number }) {
  ...
  return (
    <FadeIn delay={index * 0.06} className="soft-card group p-6 transition duration-500 hover:-translate-y-1">
      <div className="flex h-11 w-11 items-center justify-center rounded-full bg-ink text-gold-soft">
        <Icon className="h-5 w-5" strokeWidth={1.5} />
      </div>
      <p className="mt-6 text-[11px] uppercase tracking-[0.22em] text-stone">{stat.label}</p>
      <p className="mt-2 font-serif text-3xl text-ink md:text-4xl">{stat.value}</p>
    </FadeIn>
  )
}
...
export function PropertyStats({ listing }: { listing: Listing }) {
  return (
    <section className="brochure-page bg-warm px-6 py-16 md:px-12 md:py-20 lg:px-16">
      <div className="mx-auto flex min-h-[calc(100vh-8rem)] max-w-6xl flex-col justify-center">
        <FadeIn>
          <p className="eyebrow">About the home</p>
          <h2 className="editorial-display mt-4 max-w-3xl text-4xl text-ink md:text-6xl">
            {listing.headline}
          </h2>
        ...
        <FadeIn delay={0.1}>
          <p className="mt-10 max-w-3xl font-serif text-xl leading-[1.7] text-ink-soft md:text-2xl md:leading-[1.75]">
            {listing.about}
          </p>
        </FadeIn>
