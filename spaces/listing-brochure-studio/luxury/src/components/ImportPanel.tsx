...
export function ImportPanel({
  ...
  const [status, setStatus] = useState('Paste a Compass listing URL to generate the brochure.')
    ...
    <section className="relative min-h-screen overflow-hidden bg-ink text-white">
      ...
      <div className="relative z-10 mx-auto flex min-h-screen max-w-3xl flex-col justify-center px-6 py-16">
        ...
        <p className="mt-5 max-w-2xl text-base leading-relaxed text-white/70 md:text-lg">
          Submit a Compass listing link. We’ll pull photos and facts, then compose an editorial brochure you can
          present to Silicon Valley sellers — or load the demo and refine in Edit brochure.
        </p>
        ...
        <div className="mt-10 rounded-[2rem] border border-white/10 bg-white/5 p-6 backdrop-blur-md md:p-8">
          ...
          {source && source !== 'unknown' && (
            <p className="mt-3 text-sm text-gold-soft">
              ...
              {source !== 'compass' ? ' — use a Compass URL for import' : ''}
            </p>
          )}

          <details className="mt-5 rounded-2xl border border-white/10 bg-black/20 px-4 py-3">
            <summary className="cursor-pointer text-sm text-white/80">RapidAPI key (required for Compass import)</summary>
            <input
              type="password"
              value={apiKey}
            ...
            <p className="mt-2 text-xs leading-relaxed text-white/50">
              Stored in this browser session only. Subscribe to the Compass Real Estate Data API on RapidAPI, then
              paste your key here.
            </p>
          </details>
        ...
        <p className="mt-8 text-xs text-white/40">
          Photos and facts can also be added manually in Edit brochure after import or demo load.
        </p>
      </div>
    </section>
  )
